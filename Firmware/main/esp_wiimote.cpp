...
enum
{
	...
	WIIMOTE_REPORT_CORE_BUTTONS_ACC_IR10_EXT6 = 0x37
};
	...
	void Tick()
	{
		...
		case STATE_CAMERA_ENABLE:
			if (WriteAck == WriteReq)
			{
				uint8_t CameraEnable = 0x08;
				WriteToRegister(0xB00030, &CameraEnable, 1);
				SetState(STATE_EXT_ENABLE1);
			}
			break;
		case STATE_EXT_ENABLE1:
			if (WriteAck == WriteReq)
			{
				uint8_t ExtensionEnable = 0x55;
				WriteToRegister(0xA400F0, &ExtensionEnable, 1);
				SetState(STATE_EXT_ENABLE2);
			}
			break;
		case STATE_EXT_ENABLE2:
			if (WriteAck == WriteReq)
			{
				uint8_t ExtensionEnable = 0x00;
				WriteToRegister(0xA400FB, &ExtensionEnable, 1);
		...
		case STATE_SET_REPORT_MODE:
			if (WriteAck == WriteReq)
			{
				RequestReportMode(WIIMOTE_REPORT_CORE_BUTTONS_ACC_IR10_EXT6);
				SetState(STATE_OPEN);
			}
			break;
		case STATE_OPEN:
	...
	virtual void RecieveData(L2CAPConnection *Connection, MessageParser &Parser)
	{
		...
		case WIIMOTE_REPORT_ACKNOWLEDGE:
		{
			...
			if (ReportAck == WIIMOTE_REPORT_WRITE_MEMORY)
				WriteAck++;
			if (ErrorCode != 0 && ErrorCode != 7) // 7 is probably extension port being empty
		...
		case WIIMOTE_REPORT_CORE_BUTTONS_ACC_IR12:
		{
			...
			for (int i = 0; i < 4; i++)
			{
				Data.IRSpot[i].X = ((Spot[i] >> 0) & 0xFF) + ((Spot[i] >> 12) & 0x300);
				Data.IRSpot[i].Y = ((Spot[i] >> 8) & 0xFF) + ((Spot[i] >> 14) & 0x300);
				Data.IRSpot[i].Size = ((Spot[i] >> 16) & 0xF);
				Data.IRSpot[i].Size |= (Data.IRSpot[i].Size << 4); // Extend to 8-bit
			}
			Data.FrameNumber++;
			break;
		}
		case WIIMOTE_REPORT_CORE_BUTTONS_ACC_IR10_EXT6:
		{
			uint16_t Buttons = Parser.ReadWord("Buttons");
			uint8_t AccelX = Parser.ReadByte("AccelX");
			uint8_t AccelY = Parser.ReadByte("AccelY");
			uint8_t AccelZ = Parser.ReadByte("AccelZ");
			uint32_t Spot[4];
			uint32_t ProSticksAndTriggers;
			uint16_t ProButtons;
			Spot[0] = Parser.ReadTri("Spot0");
			Spot[1] = Parser.ReadWord("Spot1");
			Spot[2] = Parser.ReadTri("Spot2");
			Spot[3] = Parser.ReadWord("Spot3");
			ProSticksAndTriggers = Parser.ReadQuad("ProSticks");
			ProButtons = Parser.ReadWord("ProButtons");
			...
			for (int i = 0; i < 4; i += 2)
			{
				Data.IRSpot[i + 0].X = ((Spot[i + 0] >> 0) & 0xFF)  +  ((Spot[i + 0] >> 12) & 0x300);
				Data.IRSpot[i + 0].Y = ((Spot[i + 0] >> 8) & 0xFF)  +  ((Spot[i + 0] >> 14) & 0x300);
				Data.IRSpot[i + 0].Size = 0;
				Data.IRSpot[i + 1].X = ((Spot[i + 1] >> 0) & 0xFF)  +  ((Spot[i + 0] >> 8)  & 0x300);
				Data.IRSpot[i + 1].Y = ((Spot[i + 1] >> 8) & 0xFF)  +  ((Spot[i + 0] >> 10) & 0x300);
				Data.IRSpot[i + 1].Size = 0;
			}
			(void)ProSticksAndTriggers;
			Data.ClassicButtons = ~ProButtons;
